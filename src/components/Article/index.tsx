...
const PoemArticle: React.FC<Props> = ({ post }) => {
  return (
    <Link href={`/poemas/${post.slug}`} className="post" data-tags={post.tags}>
      <article className="relative flex h-48 w-full cursor-pointer items-center justify-center overflow-hidden rounded-lg bg-gray-dark shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl active:translate-y-0">
        <ViewTransition name={`post-${post.slug}-thumb`}>
